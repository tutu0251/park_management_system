// =============================================================================
// node_reader_uart.cpp — cooperative line parser for payment-terminal USART
// =============================================================================
//
// USART ROLE
// ----------
// Payment terminals speak a simple line protocol here; this MCU translates those lines into
// RF structs defined in park_rf_protocol.h. Anything unrelated to SWIPE/MODE is ignored so
// stray debug chatter cannot wedge the parser.
//
// NON-BLOCKING DISCIPLINE
// -----------------------
// Arduino’s HardwareSerial ISR fills a ring buffer; `node_reader_uart_poll` consumes **at most**
// the bytes currently available each loop iteration. Never use `readStringUntil` — it busy-spins.
//
// LINE GRAMMAR (examples)
// -----------------------
//   SWIPE,A1B2C3D4
//       • Hex payload must have even length, ≤16 nibbles (≤8 bytes) to fit RF card_id[].
//       • On parse failure we emit `RESULT,Error,0` so staff sees a hard fault vs silence.
//   MODE,MAINTENANCE | MODE,ONLINE | ...
//       • Updates local MachineStatus + pushes STATUS_PUSH over RF for remote dashboards.
//   STATUS,<keyword> — legacy alias identical to MODE handling.
//
// OUTPUT PATH
// -----------
// Billing outcomes return as `RESULT,<Success|Fail|Error>,<credit>\r\n` — CRLF keeps compatibility
// with both strict LF parsers and classic CR/LCD UART adapters.
//
// =============================================================================
